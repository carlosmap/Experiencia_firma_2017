...
$sql_info_prorrog="select (U.apellidos+' '+U.nombre) nombreU,U.unidad, EFProrrogas.*, EFItems_Prorrogas_Adicionales.prorroga_adicion, EFDocumentos_Soporte.documento_soporte from 
EFProrrogas  
...
if($datos_proffo=mssql_fetch_array($cur_proffo))
{
	...
	$Finicio=$datos_proffo["fecha_inicio"];
	$Final=$datos_proffo["fecha_final"];
...
echo $Prorroga.",'".$Finicio."','".$Final."',".$Documento." ,".$Valor.",'".$Observaciones."'," . $_SESSION["sesUnidadUsuario"] . ",getdate()"  ;
...
//							AND id_valores_proyecto=( select MAX(id_valores_proyecto) from EFValores_Contrato where id_proyecto=".$Proy.")";
							...
							$sqlIn1 = " DELETE FROM EFConsorcios_Empresas WHERE id_proyecto=".$Proy." AND id_valores_proyecto=".$datos_prorroga["id_valores_proyecto"]." and valores_actuales=1 ";
							$cursorIn1 = mssql_query($sqlIn1);		
echo $sqlIn1."<br>".mssql_get_last_message()."-----<BR>";																																							
							if($cursorIn1!="")
							{		
...
echo $sqlIn1."<br>".mssql_get_last_message()."-----<BR>";													
				...
				{
					//CONSULTA EL ID DEL ULTIMO REGISTRO ASOCIADO AL VALOR DEL PROYECTO, QUE FUE AFECTADO POR LA PRORROGA
					$sql_max_consorc="select max(id_consorcios_empresas) max_id from EFConsorcios_Empresas  where id_proyecto=".$Proy." and id_valores_proyecto =(
					...
					if($cursorIn1!="")
					{
						...
						//ELIMINA LOS REGISTROS ASOCIADOS, DE LAS EMPRESAS CUYO VALOR FUEA AFECTADO POR LA PRORROGA
						$sqlIn1 = " DELETE FROM EFConsorcios_Empresas WHERE id_proyecto=".$Proy." AND  id_valores_proyecto =(
										select id_valores_proyecto from EFValores_Contrato where id_prorroga_adicion=".$prorro." and tipo=1 and id_proyecto=".$Proy.") ";
						$cursorIn1 = mssql_query($sqlIn1);	
						if($cursorIn1!="")
						{				
							...
							while($datos_empre_desp=mssql_fetch_array($cursorIn1))
							{
								...
								{										
									...
									$sql_up=" UPDATE EFConsorcios_Empresas set  valor_final_contrato=".$valor_final_contrato.", valor_contrato_porcentaje=".$valor_contrato_porcentaje."
									valor_final_porcentaje=".$valor_final_porcentaje." where id_consorcios_empresas=".$datos_empre_desp["id_consorcios_empresas"]." and id_proyecto=".$Proy."
									 ";
...
/*		
				//ACTUALIZA EL VALOR DEL CONTRATO MAS RECIENTE, A ACTIVO
				$sqlIn1 = " UPDATE EFValores_Contrato SET valores_actuales=1 WHERE id_proyecto=".$Proy." AND id_prorroga_adicion=".$datos_prorroga["id_prorroga_adicion"];
				$cursorIn1 = mssql_query($sqlIn1);	

		$sqlIn1 = " UPDATE EFProrrogas SET  WHERE id_proyecto=".$Proy." AND id_prorroga=".$prorro;
		$cursorIn1 = mssql_query($sqlIn1);					
					
			//ELIMINA la EL VALOR ADICIONADO AL VALOR DEL CONTRATO	ASOCIADA A LA PRORROGA
			$sqlIn1 = " DELETE FROM EFValores_Contrato WHERE id_proyecto=".$Proy." AND id_prorroga_adicion=".$prorro;
			$cursorIn1 = mssql_query($sqlIn1);		
...
/*
	echo ("<script>window.close(); MM_openBrWindow('infoG.php?Proy=".$Proy."&sec=1-2-2&imen=1','EF','toolbar=yes,scrollbars=yes,resizable=yes,width=950,height=600');</script>");		
*/	
}
...
function valida()
{
	...
	var campos_tex = ["Prorroga","Finicio","Final","Documento","Observaciones"];	
...
<form id="formulario" name="formulario" method="POST">
  ...
  <div class="form-group" id="">
    ...
    <div class="desabilitados">
         ...
         <input type="hidden" value="<?=$datos_pro_fecha["fecha_inicio_proyecto"] ?>" name="FechaFinaProy" id="FechaFinaProy">         
    </div>
  </div>  
