...
//FUNCION PARA MOSTRAR EL CALENDARIO DE FECHA DE SOLICITUD	
	$(document).ready(function(){

		var date_input=$('input[name="FSolicitud"]'); //our date input has the name "date"
		var container=$('.bootstrap-iso form').length>0 ? $('.bootstrap-iso form').parent() : "body";
		date_input.datepicker({
			format: 'mm/dd/yyyy',
			container: container,
...
//FUNCION PARA MOSTRAR EL CALENDARIO DE FECHA DE Recepcion	
	$(document).ready(function(){

		var date_input=$('input[name="FRecepcion"]'); //our date input has the name "date"
		var container=$('.bootstrap-iso form').length>0 ? $('.bootstrap-iso form').parent() : "body";
		date_input.datepicker({
			format: 'mm/dd/yyyy',
			container: container,
...
//FUNCION PARA MOSTRAR EL CALENDARIO DE FECHA DE Facturacion	
	$(document).ready(function(){

		var date_input=$('input[name="FFacturacion"]'); //our date input has the name "date"
		var container=$('.bootstrap-iso form').length>0 ? $('.bootstrap-iso form').parent() : "body";
		date_input.datepicker({
			format: 'mm/dd/yyyy',
			container: container,
...
/*
//funcion para el campo de fechas	
function calendario(campo)
{
//	alert(campo);
	//FUNCIONES PARA MOSTRAR EL CALENDARIO EN LOS CAMPOS DE FECHA "Final"	
//	$(document).ready(function(){
//		var date_input=campo;
		var date_input=$('input[name="'+campo+'"]'); //our date input has the name "date"
//		var date_input=$(''+campo+''); 
		var container=$('.bootstrap-iso form').length>0 ? $('.bootstrap-iso form').parent() : "body";
		date_input.datepicker({
			format: 'mm/dd/yyyy',
...
//1= VALIDACION CAMPOS DE TEXTO (array_campos= ARRAY DE LOS CAMPOS DE TEXTO A VALIDAR), (tipo=1 PARA CAMPOS DE TEXTO)
//2= VALIDACION DE CAMPOS TIPO CHECKBOX (array_campos= ARRAY CON EL NOMBRE DEL CHECKBOX A VALIDAR, PARA ESTE CASO SE ENVIA UN SOLO ELEMENTO EN EL ARRAY)
//	, (tipo=1 PARA CAMPOS CHECKBOX), (cantidad_items= CANTIDAD DE CHECKBOX A VALIDAR )
...
//VALIDACION DE LA EXTENCION DE UN DOCUMENTO ADJUNTO
//archivo= ARCHIVO QUE SE PRETENDE SUBIR
//array_ext_permiti = ARRAY CON LAS EXTENCIONES PERMITIAS DEL DOCUMENTO A SUBIR
//campo = NOMBRE DEL CAMPO A VALIDAR
function valida_extension(archivo,array_ext_permiti, campo)
{		
//alert("help"+campo+"Ext");
	document.getElementById("help"+campo+"Ext").style.display="none";

	var error=1;
	
	var extension = (archivo.substring(archivo.lastIndexOf("."))).toLowerCase(); 		
//alert(extension)	;
	for(var z=0; z<= array_ext_permiti.length; z++)
	{
		//SI LA EXTENCION DEL ARCHIVO ES PERMITIDA 
		if(array_ext_permiti[z]==extension)
		{
			error=0;			
			break;
		}
	...
	if(error==1)
	{		
		document.getElementById("div"+campo).className="form-group has-error";	
		document.getElementById("help"+campo+"Ext").style.display="inline-block";			
	}
	return error;		
}