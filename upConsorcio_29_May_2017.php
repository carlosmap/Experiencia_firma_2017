	...
	{	
		for($i=0;$i<=$cantEmpre;$i++)		
		{
			...
			if($$Empresa!="")
			{
				...
				, id_proyecto, id_empresa, id_consorcio, porcentaje_participacion, empresa_lider
				, valor_contrato_porcentaje, valor_final_contrato, valor_final_porcentaje, valores_actuales, usuarioGraba, fechaGraba) VALUES";
				
				$sqlIn1 = $sqlIn1." (  (select isnull(MAX(id_consorcios_empresas),0)+1 id_consorc  from EFConsorcios_Empresas WHERE id_proyecto=".$Proy." ), 
				".$Proy.", ".$$Empresa." ,".$id_consorcio.", ".$$porcentaje." , ".$lider." ,
				".$valorParticipacion.", ".$valorP.", ".$valorParticipacion.", 1,  ".$_SESSION["sesUnidadUsuario"].", getdate() ) ";
				$cursorIn1 = mssql_query($sqlIn1);	
			}
//echo $sqlIn1."<br>".mssql_get_last_message()."<br>";					
		}
...
$sql_empresas_conso="select EFEmpresas.empresa, EFConsorcios_Empresas.*, CASE EFConsorcios_Empresas.id_empresa when ".$datos_encargada["id_empresa"]." then '1' else '0' end encargada  from EFConsorcios_Empresas
...
<div class="container-fluid" >
  ...
  <div class="form-group" id="divNomConso">
    ...
    <input type="text" class="form-control" id="NomConso" name="NomConso" value="<?=$datos_nom_consr["nombre_consorcio"] ?>"  placeholder="Nombre del consorcio" size="20px" autofocus>
...
?>        
    <tr class="TR" >
    	<td>
<?php
			//si la empresa que se esta recorriendo es la empresa encargada
			if($datos_empresas_conso["id_empresa"]==$datos_encargada["id_empresa"])
			{
?>    
...
<?php
			...
			{
?>
				<select name="Empresa<?=$z ?>" id="Empresa<?=$z ?>" class="form-control">
<?php 			
				...
				{
		?>             
					<option value="<?=$datos_empre["id_empresa"] ?>"><?=$datos_empre["empresa"]  ?></option>';
		<?php
...
?>
          ...
          <td><input type="text" name="porcentaje<?=$z ?>" id="porcentaje<?=$z ?>" value="<?=$datos_empresas_conso["porcentaje_participacion"] ?>" size="4" maxlength="5" onKeyPress="return acceptNumD(event)" onBlur="sumPorcentaje()" > 
          %</td>
          <td>$ 
            <input type="text" class="desabilitados2" name="valorParticipacion<?=$z ?>"  value="<?=$datos_empresas_conso["valor_contrato_porcentaje"] ?>" id="valorParticipacion<?=$z ?>" size="25"  onKeyPress="return acceptNumD(event)"  disabled ></td>
          <td><input type="radio" name="lider" onClick="regLider(<?=$z ?>)" id="lider"></td>
...
<?php
			...
			{
?>          
				<td><i class="glyphicon glyphicon-minus-sign btn btn-danger" onClick="delFila("<?=$z ?>"); sumPorcentaje();"></i></td>
<?php
			}
?>
    </tr>
