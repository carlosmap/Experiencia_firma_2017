...
$sql_max_fech_prorroga="select CONVERT(varchar,fecha_ultima_facturacion,101) fechafinal, CONVERT(varchar,fecha_inicio_facturacion,101) fecha_inicio_fact from EFValores_Facturados  where id_proyecto=".$Proy ." and id_valor_facturado=(select MAX(id_valor_facturado) from EFValores_Facturados  where id_proyecto=".$Proy .")";
...
//alert(" -- "+document.getElementById("Finicio").value+ " *** "+document.getElementById("FechaIniProy").value) ;	
	...
	if(document.getElementById("FechaUltProrroga").value!="")
	{
		...
		//SI LA FEHCA DE FINALIZACION DEL PRORROGA ES INFERIOR A LA FECHA DE FINALIZACION DE LA ULTIMA PRORROGA RGISTRADA
		if (compare_fecha( document.getElementById("FechaUltProrroga").value , document.getElementById("Final").value))
		{  
	//alert("Ingresaa ");
			document.getElementById("divFinicio").className="form-group has-error";		
...
<?php
		if($datos_max_fech_prorroga["fecha_inicio_fact"]=="")
		{
?>            
...
?>
				<div class="input-group">
	                <input class="form-control" id="" name="" value="<?=$datos_max_fech_prorroga["fecha_inicio_fact"] ?>"  size="26" readonly  type="text"/>
                    <input type="hidden" id="Finicio" name="Finicio" value="<?=$datos_max_fech_prorroga["fecha_inicio_fact"] ?>" >
                </div>
<?php			
		}
?>           
	...
	<div class="container-fluid">
	 ...
	 <span id="helpFinicioProrr2" class="help-block" style="display:none;" >La fecha de finalizaci&oacute;n, no puede ser inferior a la fecha de finalizaci&oacute;n de la ultima facturaci&oacute;n registrada (<?=$datos_max_fech_prorroga["fechafinal"] ?>).</span>          
