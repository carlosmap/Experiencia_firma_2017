...
if($recarga==2)
{
	...
	if($acc==2)
	{
		//SI EL ESTADO DEL PROYECTO ES SUSPENDIDO, SOLO SE PERMITE CAMBIAR LA FECHA DE FINALIZACION 
		if((($EstadoAnt==5)&&($TipoPAnt==4) ))		
		{
			mssql_query("begin transaction");	
			//SI SE HA CAMBIADO LAS FECHAS INICIO / FINALIZACION
			if( ( ($Finicio!=$FinicioAnt)||($Final!=$FinalAnt) ))
			{		
				...
				{								
					...
					values( (select isnull(MAX(id_fecha_proyecto),0)+1 id_proye  from EFFechas_Proyecto WHERE id_proyecto=".$Proy." ) , ".$Proy." ,NULL, '".$FinicioAnt."', '".$Final."', 1 ," . $_SESSION["sesUnidadUsuario"] . ",getdate() )";
					$cursorIn1 = mssql_query($sqlIn1);		
...
?>		
		...
		//SI LA FEHCA DE FINALIZACION DEL PROYECTO ES INFERIOR A LA FECHA DE FINALIZACION INICIAL DEL PROYECTO
		if (compare_fecha( document.getElementById("FinalAnt").value , document.getElementById("Final").value))
		{  
...
//---
$dato_val_if=mssql_fetch_array(mssql_query(" select COUNT(*) cantA from EFLiquidacion WHERE id_proyecto=".$Proy));
$cant_liquidacion=( (int) $dato_val_if["cantA"]);

$dato_val_if=mssql_fetch_array(mssql_query(" select COUNT(*) cantA from EFReanudaciones WHERE id_proyecto=".$Proy));
$cant_reanudaciones=( (int) $dato_val_if["cantA"]);

$dato_val_if=mssql_fetch_array(mssql_query(" select COUNT(*) cantA from EFSuspenciones WHERE id_proyecto=".$Proy));
$cant_suspenciones=( (int) $dato_val_if["cantA"]);
...
//REALIZA LA SUMATORIA DEL LA CANTIDAD DE PRORROGAS, ADICIONES, VALORES FACTURADOS, LIQUIDACION, REANUDACIONES Y SUSPENCIONES
$tota_valoresF_prorroga_adiciones=$cant_valores_f+$cant_prorrogas+$cant_adiciones+$cant_liquidacion+$cant_reanudaciones+$cant_suspenciones;

//SI SE VA A ELIMINAR LA INFORMACION DEL PROYECTO O (SI EL ESTADO DEL PROYECTO ES SUSPENDIDO Y ES UN PROYECTO FACTURABLE)
//SE DESACTIVARAN LOS CAMPOS DE LA PAGINA
$dis="";

///****************** TEMPORAR PÁRA DESCOMENTARIAR (06 jUL 2017) *********************** 								
//if(($acc==3)||(($EstadoAnt==5)&&($TipoPAnt==4) ))
if($acc==3)
	$dis="disabled";
...
?>
    ...
    <?php
		//SI EL ESTADO DEL PROYECTO ES ADJUDICADO, SE CONSULTAN TODOS LOS ESTADOS ACTIVOS, MENOS SUSPENDIDO y LIQUIDADO
		if($Estado==2)		
		{
			///****************** TEMPORAR PÁRA DESCOMENTARIAR (06 jUL 2017) *********************** 								
//	        $cur_esta=mssql_query("select * from EFEstados_Proy where estado=1 and id_tipo_proyecto=".$TipoP." and  id_estado_proy not in (5,11) order by estado_proyecto");

			$cur_esta=mssql_query("select * from EFEstados_Proy where estado=1 and id_tipo_proyecto=".$TipoP."  order by estado_proyecto");
		}
		//SI EL ESTADO DEL PROYECTO NO  ES ADJUDICADO, SE CONSULTAN TODOS LOS ESTADOS ACTIVOS, MENOS ADJUDICADO, SUSPENDIDO y LIQUIDADO
		if($Estado!=2)		
		{
			///****************** TEMPORAR PÁRA DESCOMENTARIAR (06 jUL 2017) *********************** 											
//	        $cur_esta=mssql_query("select * from EFEstados_Proy where estado=1 and id_tipo_proyecto=".$TipoP." and  id_estado_proy not in (2,5,11) order by estado_proyecto");

			$cur_esta=mssql_query("select * from EFEstados_Proy where estado=1 and id_tipo_proyecto=".$TipoP." and  id_estado_proy not in (2) order by estado_proyecto");
		}
  ...
  <div class="form-group" id="divObservaciones">
    ...
    <textarea name="observacion" class="form-control" id="observacion" cols="30" rows="1" <?=$dis ?>><?=$observacion ?></textarea>
...
?>
            <div class="desabilitados">
                <?=$Finicio ?>&nbsp; 
                  <i class="glyphicon glyphicon-info-sign" title="No se permite cambiar la fecha, porque ya se han registrado prorrogas, adiciones, valores facturados,  liquidaci&oacute;n, suspensiones o reanudaciones. ">
                  </i>            
            </div>
...
<?PHP
		//SI SE PERMITE CAMBIAR LA FECHA O (SI EL ESTADO ES SUSPENDIDO Y EL TIPO DE PROYECTO ES FACTURABLE)
		if(($disab=="")||( ($EstadoAnt==5)&&($TipoPAnt==4) ))
		{
...
<?php				
			...
			{
				//SI EL ESTADO DEL PROYECTO ES SUSPENDIDO, SE PERMITE CAMBIAR LA FECHA DE FINALIZACION, YA QUE PUEDE DARSE EL CASO QUE LA SUSPENCION SE PRORROGUE 
				//TANTO, AL PUNTO DE ALCANSAR LA FECHA DE FINALIZACION DEL PROYECTO, POR ESTA RAZON SE PERMITE CAMBIAR LA FECHA DE FINALIZACION
?>              
               <input class="form-control" id="Final" name="Final" placeholder="MM/DD/YYYY"  value="<?=$Final ?>" readonly type="text" <? if(($EstadoAnt!=5)){ echo $dis; } ?> />
                <div class="input-group-addon">
                  <i class="glyphicon glyphicon-calendar">
                  </i>
                </div>               
...
<?php
	...
	{
?>	
		<span id="helpFinal3" class="help-block" style="display:none;" >La fecha de finalizaci&oacute;n del proyecto no puede ser menor a la fecha de finalizaci&oacute;n anterior (<?=$FinalAnt ?>), por favor verifique.</span>     	
<?php
	}
?>          
  </div>
...
<?PHP
		...
		{	
			...
			if($tota_valoresF_prorroga_adiciones>0)
			{
				...
				$disab="readonly ";
				$mensa="No se permite cambiar la forma de ejecuci&oacute;n, porque ya se han registrado prorrogas, adiciones, valores facturados, liquidaci&oacute;n, suspensiones o reanudaciones.";
			}			
		}
		...
		{	
			...
			if(($tota_valoresF_prorroga_adiciones>0)|| ($cant_consorcios>0))
			{
				//SE DEJARA EL CAMPO COMO SOLO LECTURA Y NO SE PODRA CMABIAR LA FORMA DE EJECUCION
				$disab="readonly ";
				$mensa="No se permite cambiar la forma de ejecuci&oacute;n, porque ya se han registrado prorrogas, adiciones, valores facturados, liquidaci&oacute;n, suspensiones, reanudaciones o un consorcio.";				
			}					
		}
...
?>
            <div class="desabilitados">
                <?=$Valor ?>&nbsp; 
                  <i class="glyphicon glyphicon-info-sign" title="No se permite cambiar el valor, porque ya se han registrado prorrogas, adiciones, valores facturados,  liquidaci&oacute;n, suspensiones, reanudaciones o un consorcio. ">
                  </i>                
            </div>
