...
$sql_nom="
select * from (
	select unidad,upper(nombre) nombre,upper(apellidos) apellidos from HojaDeTiempo.dbo.Usuarios 
	where retirado is null and fechaRetiro is null 
	union
	select unidad_director_coordinador,upper(nombre) nombre,upper(apellidos) apellidos from EFDirector_Coordinador 
	inner join HojaDeTiempo.dbo.Usuarios U on EFDirector_Coordinador .unidad_director_coordinador=U.unidad
	where director_coordinador_actual=1 and id_proyecto=".$Proy."
	union
	select unidad_ordenador,upper(nombre) nombre,upper(apellidos) apellidos from EFOrdenadores_gasto 
	inner join HojaDeTiempo.dbo.Usuarios U on EFOrdenadores_gasto.unidad_ordenador=U.unidad
	where ordenador_actual=1 and id_proyecto=".$Proy."
)A	
order by apellidos ";
